# NOTE: tuples, mnist, still in testing, params haven't been set. 
...
current_date = datetime.now().strftime("%Y_%m_%d_outwards_mnist")
save_dir = f'evaluation/{current_date}_tuples/{target_digit}'
...
def generate_images(model, channel_n=CHANNEL_N, seed_std=SEED_STD, iter_n=ITER_N):
    x = make_seed(16, channel_n=channel_n, seed_std=seed_std)  # Seed for 16 images
...
test_params_bundles = [test_params1, test_params2, test_params3, test_params4, test_params5] 
...
with open(output_text_file, 'w') as log_file:
    ...
    for test_params_bundle in test_params_bundles:
        print(f'Testing {test_params_bundle}s', file=log_file)
        current_params = test_params_bundle.copy()
        likelihoods = []
        param_saved_vals = []
        ...
        print(f"Loss {likelihood}", file=log_file)
